                ...
                (too-long-lines-overlay-display
                 (lambda (o num-hidden)
                   (if (eq (overlay-get o 'too-long-line) 'shown)
                     ...
                     (overlay-put o 'display (concat "... " (prin1-to-string num-hidden) " hidden characters"))
            ...
            (when (> line-length too-long-lines-threshold)
              ...
              (dolist (ov (overlays-in line-beg line-end))
                (if (overlay-get ov 'too-long-line)
                    (if (> line-end (overlay-end ov))
                        (progn (setq cur-too-long-line (overlay-get ov 'too-long-line))
                               (delete-overlay ov))
                      (move-overlay ov (+ line-beg too-long-lines-show-number-of-characters) line-end)
                      (funcall too-long-lines-overlay-display ov (- line-length too-long-lines-show-number-of-characters))
